...
function wrongParamType(name, type) {
  ...
  return 'Parameter '+name+' has to be '+verb+' '+type;
}
  ...
  return function(req, res, next) {
    ...
    var check = params.every(function(p) {
      ...
      if((param === undefined || param === null) && p.required) {
        message = 'Parameter '+p.name+' is required in '+p.in;
        return false;
      }
      ...
      if(p.type === 'number' || p.type === 'integer') {
        ...
        if(p.maximum) {
          if(p.exclusiveMaximum) {
            if(param >= p.maximum) {
              message = 'Parameter '+p.name+' has to be lower than '+p.maximum;
              return false;
            }
          } else {
            if(param > p.maximum) {
              message = 'Parameter '+p.name+' has to be lower or equals than '+p.maximum;
              return false;
            }
          }
        }
        ...
        if(p.minimum) {
          if(p.exclusiveMinimum) {
            if(param <= p.minimum) {
              message = 'Parameter '+p.name+' has to be greater than '+p.minimum;
              return false;
            }
          } else {
            if(param < p.minimum) {
              message = 'Parameter '+p.name+' has to be greater or equals than '+p.minimum;
              return false;
            }
          }
        }
        ...
        if(p.multipleOf) {
          if(param%p.multipleOf !== 0) {
            message = 'Parameter '+p.name+' has to be a multiple of '+p.multipleOf;
            return false;
          }
        }
      }
      ...
      if(p.type === 'string') {
        ...
        if(p.maxLength) {
          if(param > p.maxLength) {
            message = 'Parameter '+p.name+' has to be a length lower or equals than '+p.maxLength;
            return false;
          }
        }
        // minimum length
        if(p.minLength) {
          if(param < p.minLength) {
            message = 'Parameter '+p.name+' has to be a length greater or equals than '+p.minLength;
            return false;
          }
        }
        // regex test
        if(p.pattern) {
          ...
          if(!re.test(pattern)) {
            message = 'Parameter '+p.name+' has to match the following expression: '+p.pattern;
            return false;
          }
        }
        // in enum
        if(p.enum) {
          if(_.findIndex(p.enum, param) === -1) {
            message = 'Parameter '+p.name+' has to be in set ['+p.enum.join(',')+']';
            return false;
          }
        }
      }
      ...
      if(p.type === 'array' && param instanceof Array) {
        ...
        if(p.maxItems) {
          if(param.length > p.maxItems) {
            message = 'Parameter '+p.name+' has to be less or equals items than '+p.maxItems;
            return false;
          }
        }
        // minimum of items
        if(p.minItems) {
          if(param.length < p.minItems) {
            message = 'Parameter '+p.name+' has to be more or equals items than '+p.minItems;
            return false;
          }
        }
        // unique items
        if(p.uniqueItems) {
          if(!checkUnique(param)) {
            message = 'Parameter '+p.name+' must only have unique items';
            return false;
          }
        }
      }
